<template>
  <div :class="['knob', 'style' + knob.style]">
    ...
    <div class="knob-dial" :style="{ color: knob.active ? knob.color : '#888' }">
      <div class="dial-grip" :style="{ transform: 'translate(-50%,-50%) rotate(' + knob.rotation + 'deg)' }" @mousedown="handleMouseDown"></div>
      <svg class="dial-svg" viewBox="0 0 100 100">
        <path d="M20,76 A 40 40 0 1 1 80 76" fill="none" stroke="#55595C" />
        <path d="M20,76 A 40 40 0 1 1 80 76" fill="none" :stroke="knob.active ? knob.color : '#888'" :style="{ 'stroke-dashoffset': 184 - 184 * ((knob.rotation * 1 + 132) / 264) }" />
      </svg>
    </div>
