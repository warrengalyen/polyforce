<template>
  <div class="keyboard__wrapper">
    <ul class="keyboard" @mousedown="mouseDown = true" @mouseup="mouseDown = false" @mouseleave="mouseDown = false">
      <li v-for="key in keyNotes" class="key" :class="[...key.style, key.active ? 'active' : '']" :key="key.note" :data-note="key.note" :data-midi="key.midi" @mousedown="handleMouseDown(key)" @mouseup="handleMouseUp(key)" @mouseout="handleMouseOut(key)" @mouseover="handleMouseOver(key)">
        {{ key.active ? "T" : "F" }}
      </li>
    </ul>
    {{ mouseDown }}
