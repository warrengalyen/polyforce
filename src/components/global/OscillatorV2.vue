<template>
  <div>
    <canvas id="analyzer" style="width: 80px; height:50px; background-color: #eee;"></canvas>
    <p>
    ...
    <div class="synth-control">
      ...
      <RangeInput label="count" min="1" max="8" :value="count" @input="
        count = $event;
      osc.setOsc({ count: $event });
      " />
      <RangeInput label="phase" max="360" :value="phase" @input="
        phase = $event;
      osc.setOsc({ phase: $event });
      " />
      <RangeInput label="spread" :value="spread" @input="
        spread = $event;
      osc.setOsc({ spread: $event });
      " />
      <RangeInput label="finetune" :value="finetune" min="-100" @input="
        finetune = $event;
      osc.setOsc({ finetune: $event });
      " />
      <RangeInput label="seminote" min="-12" max="12" :value="seminote" @input="
        seminote = $event;
      osc.setOsc({ seminote: $event });
      " />
      <RangeInput label="octave" min="-3" max="3" :value="octave" @input="
        octave = $event;
      ...
      "
    />-->
      <RangeInput label="pan" min="-1" max="1" step="0.01" :value="pan" @input="
        pan = $event;
      osc.set({ pan: $event });
      " />
      <RangeInput label="gain" max="1" step="0.01" :value="gain" @input="
        gain = $event;
      osc.set({ gain: $event });
      " />
    </div>
...
export default {
  ...
  mounted() {
    ...
    */
    ...
    const waveCanvas = document.getElementById("analyzer");
